...
class Tracer
{
 ...
 public:
  ...
  {
    Trace("ff::toygb::debug") << "blame gb start: " << elems[elemId] << std::endl;
    set<size_t> visited{};
    vector<size_t> q;
    q.push_back(elemId);
    while (!q.empty())
    {
      elemId = q.back();
      Trace("ff::toygb::debug") << "blame gb: " << elemId << ": " << elems[elemId] << std::endl;
      q.pop_back();
    ...
    for (size_t id : visited)
    {
      if (id < n_inputs)
      {
        Trace("ff::toygb::debug") << "blame gb keep: " << id << ": " << elems[id] << std::endl;
        out.push_back(id);
      }
    }
    return out;
...
RingElemBlame NRLMBlame(ConstRefRingElemBlame f, const vector<RingElemBlame>& v, Tracer& tracer)
{
...
RingElemBlame NormalRemainderBlame(ConstRefRingElemBlame f, const vector<RingElemBlame>& v, Tracer& tracer)
{
  if (IsZero(f.first)) return f;
  const SparsePolyRing P = owner(f.first);
  RingElem ansNRpoly(P);
  RingElemBlame ansNR = {ansNRpoly, tracer.deduce(ansNRpoly, NO_BLAME, NO_BLAME)};
...
RingElemBlame SPolyBlame(RingElemBlame g, RingElemBlame h, Tracer& tracer)
{
  ...
  PPMonoidElem m = lcm(LPP(g.first), LPP(h.first));  // monoid: not implemented in CoCoA-5
...
vector<RingElem> toyGBasisBlame(ideal I, vector<size_t>& blame)
{
  ...
  for (const auto& f : inputs)  // minor cleaning on gens(I):
  {
    ...
    if (not(IsZero(g.first)))
    {
      GB.push_back(g);
      index_g = len(GB) - 1;
      for (long n = 0; n < index_g; ++n) pairs.push_back(GBPairBlame({n, GB[n].second}, {index_g, GB[index_g].second}));
    }
  }
  //VERBOSE(8) << "start\n  GB = " << GB << "\n  pairs = " << pairs << endl;
  //  -- MAIN LOOP -----------------------------
  ...
  while (!pairs.empty())
  {
    ...
    pairs.pop_front();
    VERBOSE(5) << "--" << p << endl;
    g = NormalRemainderBlame(SPolyBlame(GB[p.myFirstIndex().first], GB[p.mySecondIndex().first], tracer), GB, tracer);
