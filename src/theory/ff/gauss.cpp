...
Matrix::Matrix(const Integer& mod, size_t cols, size_t protCols, size_t rows)
    : d_mod(mod),
      d_zero(0, mod),
      d_cols(cols),
      d_protCols(protCols),
...
void Matrix::setEntry(size_t r, size_t c, const Ffv& v)
{
  Assert(r < d_rows);
  Assert(c < d_cols);
  Entry p{c, v};
...
void Matrix::setRow(size_t r, std::vector<std::pair<size_t, Ffv>>&& row)
{
  row.erase(std::remove_if(row.begin(),
                           row.end(),
                           [](const auto& p) { return p.second.isZero(); }),
            row.end());
#ifdef CVC5_ASSERTIONS
  for (size_t i = 0; i < row.size(); ++i)
  {
    Assert(row[i].first < d_cols);
    Assert(!row[i].second.isZero());
    if (i > 0)
    {
...
const Ffv& Matrix::getEntry(size_t r, size_t c) const
{
  Assert(r < d_rows);
  Assert(c < d_cols);
  Entry p{c, d_zero};
...
const std::vector<std::pair<size_t, Ffv>>& Matrix::getRow(size_t r) const
{
  Assert(r < d_rows);
  return d_mat[r];
}
...
void Matrix::ref()
{
  ...
  while (pr < d_rows && pc < elimCols())
  {
    ...
    using Key = std::pair<Integer, Ffv>;
    const auto mkKey = [](const Ffv& v) {
      return Key(v.toSignedInteger().abs(), v);
    };
...
bool Matrix::inRefWithPivots(std::vector<size_t>& pivotCols) const
{
  size_t pc = 0;
  size_t r = 0;
  for (; r < d_rows; ++r)
  {
    ...
    if (getEntry(r, firstNzC).toSignedInteger() != -1)
    {
      Trace("ff::gauss::form")
          << " not in REF b/c row " << r
          << " has first NZ = " << getEntry(r, firstNzC).toString()
          << std::endl;
      return false;
    }
...
void Matrix::swapRows(size_t r0, size_t r1) { std::swap(d_mat[r0], d_mat[r1]); }
void Matrix::scaleRow(size_t r, const Ffv& k)
{
  Assert(!k.toInteger().isZero());
...
void Matrix::assertNoExplicitZeros() const
{
#ifdef CVC5_ASSERTIONS
  for (size_t r = 0; r < d_rows; ++r)
  {
    ...
    {
      Assert(!v.isZero()) << "explicit zero at r=" << r << " c=" << c
                          << std::endl;
    }
  }
#endif  // CVC5_ASSERTIONS
